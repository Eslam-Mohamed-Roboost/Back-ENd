...
public record GetTeacherSubjectsBySubjectQuery(string? Subject) : IRequest<RequestResult<List<TeacherSubjectMatrixDto>>>;

public class GetTeacherSubjectsBySubjectQueryHandler(
    ...
    : RequestHandlerBase<GetTeacherSubjectsBySubjectQuery, RequestResult<List<TeacherSubjectMatrixDto>>>(parameters)
{
    public override async Task<RequestResult<List<TeacherSubjectMatrixDto>>> Handle(GetTeacherSubjectsBySubjectQuery request, CancellationToken cancellationToken)
    {
        // Get teacher IDs filtered by subject if provided
        IQueryable<long> teacherIdsQuery = teacherSubjectsRepository.Get().Select(ts => ts.TeacherId).Distinct();

        if (!string.IsNullOrWhiteSpace(request.Subject))
        {
            teacherIdsQuery = from ts in teacherSubjectsRepository.Get()
                              join s in subjectsRepository.Get() on ts.SubjectId equals s.ID
                              where s.Name.ToLower().Contains(request.Subject.ToLower())
                              select ts.TeacherId;
            teacherIdsQuery = teacherIdsQuery.Distinct();
        }

        var teacherIds = await teacherIdsQuery.ToListAsync(cancellationToken);

        if (!teacherIds.Any())
        {
            return RequestResult<List<TeacherSubjectMatrixDto>>.Success(new List<TeacherSubjectMatrixDto>());
        }

        // Get teachers - project to avoid navigation property issues
        var teachers = await userRepository.Get()
            .Where(u => u.Role == ApplicationRole.Teacher && teacherIds.Contains(u.ID))
            .Select(u => new
            {
                u.ID,
                u.Name,
